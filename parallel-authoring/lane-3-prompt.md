Read CLAUDE.md, then PLAN_AUTHORING_BRIEF.md in full, then start your own log file PLAN_AUTHORING_LOG_lane3.md, before doing anything else.
...
YOUR RANGE: author ONLY plans.json indices 824 through 1032 (inclusive). Do not touch any index outside that range. Start at 824 (or, on resume, at the next unfinished index within 824–1032 per your own log).
...
- Log to PLAN_AUTHORING_LOG_lane3.md ONLY, in the format §10 describes (indices done, slugs, skips with reasons, next index within your range). Create it if it doesn't exist.
- Stop when index 1032 is done. That's the end of your lane — don't continue past it.
