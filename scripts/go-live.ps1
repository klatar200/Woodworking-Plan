...
#   5. publish: flips @/lib/seo SITE_INDEXABLE to true and commits     (-NoPublish to skip)
...
# Native commands do not stop the script on non-zero exit by themselves - run every one
# through this so a red gate or failed merge cannot fall through to the production seed.
...
if ($branch -eq 'main') { throw "You are on main. Run this from the swap branch (e.g. kreg-swap)." }
...
if ($willPublish) {
  ...
  if ($c -match 'SITE_INDEXABLE = false') {
    $flipped = $c -replace 'SITE_INDEXABLE = false', 'SITE_INDEXABLE = true'
    [System.IO.File]::WriteAllText($seo, $flipped)
...
# 6) ship to main
if (-not $SkipMerge) {
  Step "Ship: merge $branch into main and push (triggers the production deploy)"
