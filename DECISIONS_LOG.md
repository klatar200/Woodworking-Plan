...
**Binding:** **All three actors share ONE working tree** — Keagan's checkout. No push/pull transport between Claude and Cursor; a file one writes is a file the other reads. Isolation is now by **branch**: Cursor creates `cursor/sprint-NN-<slug>` off `main`, commits there, and is limited to `checkout -b` / `add` / `commit` / `status`. **Forbidden to Cursor: push, merge, rebase, reset, stash, `checkout main`, anything `--force`** — there is one tree and no remote copy, so a destructive verb costs the real repo, not a disposable VM. Keagan retains the branch boundary: cut `changes.diff`, merge after the audit passes, push. Claude's own git ban is UNCHANGED and unrelated (sandbox mount corrupts `.git`, CLAUDE.md §5). Cursor Cloud stays available on request; `AGENTS.md` carries that branch of the procedure.
**Consequences.** The loop loses four git operations (pack push, fetch, FIXES push, fix-round pull) and the "not pushed yet" failure mode. Branch isolation preserves audit-before-merge, which was the only property Cloud's separate clone actually provided. Running on Windows makes the PowerShell 5.1 UTF-16LE redirection trap reachable for `verify.txt`, so `scripts/verify.mjs` gained `--out <path>` (Node writes UTF-8; no shell involved) and shell redirection into `verify.txt` is now forbidden. Cutting `changes.diff` is no longer optional bookkeeping — without it the audit falls back to reading `src/`, which cost several times the tokens for the same conclusion in Sprint 76.
