'use server';

import { revalidatePath } from 'next/cache';
import { redirect } from 'next/navigation';
import {
  addToShoppingList,
  addBoardDesignToShoppingList,
  removeFromShoppingList,
  removeBoardDesignFromShoppingList,
} from '@/lib/shopping-list';
import { checkRateLimit } from '@/lib/rate-limit';
import { denialTarget, bounceTarget } from '@/lib/rate-limit-feedback';
import { formString } from '@/lib/form-fields';
import { guardAction } from '@/lib/action-guard';

/**
 * Add / remove a plan or board design on the shopping list — Sprint 22 / 64.
 *
 * SECURITY: server actions are PUBLIC HTTP ENDPOINTS. Next generates an id for each and
 * anyone can POST to it; the fact that no button in the UI calls it means nothing. So no
 * user id is read from the form — lib helpers derive the owner from the verified Clerk
 * session and take no `userId`.
 *
 * RATE LIMITING mirrors like/save: `checkRateLimit('toggle')` runs FIRST, before any DB
 * work. A denied request does NO work and never throws (an uncaught throw out of a server
 * action is an HTTP 500 and a crashed page — see src/lib/rate-limit.ts). It `redirect()`s
 * back with `?notice=slow-down` instead, a framework-handled 303. `denialTarget` runs the
 * `returnTo` field through `safeReturnTo`, so the bounce cannot become an open redirect.
 *
 * Sprint 64: exactly one of planId / boardDesignId. Both set ⇒ silent bounce (never throw;
 * a DB CHECK violation would throw and §7 forbids that).
 */

/**
 * MALFORMED INPUT IS DROPPED, NOT THROWN (2026-07-19) — see src/lib/form-fields.ts.
 * The old local `requiredString` threw, and an uncaught throw out of a server action is
 * an HTTP 500.
 */
export async function addToShoppingListAction(formData: FormData): Promise<void> {
  if (!(await checkRateLimit('toggle'))) redirect(denialTarget(formData, '/'));

  const planId = formString(formData, 'planId');
  const boardDesignId = formString(formData, 'boardDesignId');
  // Exactly one — reject both before the DB CHECK can throw.
  if (planId !== null && boardDesignId !== null) {
    redirect(bounceTarget(formData, '/'));
  }
  if (planId === null) redirect(bounceTarget(formData, '/'));

  const slug = formData.get('slug');

  // AUDIT FIX 2026-07-19: `requireUser()` inside the lib throws for an expired
  // session, which escaped as an HTTP 500 on a public page. See action-guard.ts.
  await guardAction(addToShoppingList(planId), formData, '/');

  if (typeof slug === 'string' && slug !== '') revalidatePath(`/plans/${slug}`);
  revalidatePath('/shopping-list');
}

export async function addBoardDesignToShoppingListAction(
  formData: FormData,
): Promise<void> {
  if (!(await checkRateLimit('toggle'))) {
    redirect(denialTarget(formData, '/designer'));
  }

  const designId = formString(formData, 'designId');
  const planId = formString(formData, 'planId');
  // Exactly one — a forged planId alongside designId must not reach the DB.
  if (designId === null || planId !== null) {
    redirect(bounceTarget(formData, '/designer'));
  }
  // Ignore any forged owner id in the form — ownership is verified in the lib.
  void formData.get('userId');

  await guardAction(addBoardDesignToShoppingList(designId), formData, '/designer');

  revalidatePath('/shopping-list');
  revalidatePath(`/designer/${designId}`);
  redirect('/shopping-list');
}

export async function removeFromShoppingListAction(formData: FormData): Promise<void> {
  if (!(await checkRateLimit('toggle'))) redirect(denialTarget(formData, '/'));

  const planId = formString(formData, 'planId');
  const designId = formString(formData, 'designId');
  if (planId !== null && designId !== null) {
    redirect(bounceTarget(formData, '/shopping-list'));
  }

  const slug = formData.get('slug');

  if (designId !== null) {
    await guardAction(
      removeBoardDesignFromShoppingList(designId),
      formData,
      '/shopping-list',
    );
    revalidatePath('/shopping-list');
    return;
  }

  if (planId === null) redirect(bounceTarget(formData, '/'));

  await guardAction(removeFromShoppingList(planId), formData, '/');

  if (typeof slug === 'string' && slug !== '') revalidatePath(`/plans/${slug}`);
  revalidatePath('/shopping-list');
}
