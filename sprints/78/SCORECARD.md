...
- A2 | PASS | imports only `@/lib/format` + `@/lib/board-designer/*`; source guard; `verify.txt:136`
- A3 | PASS | `verify.txt:136` — shape test asserts non-empty fields + `quantities` array
- A4 | PASS | literal slug ids in `build-steps.ts`; source guard rejects `randomUUID` / index templates
- A5 | PASS | `verify.txt:136` — deep-equal determinism test

## Edge grain (T2)

- A6 | PASS | `verify.txt:136` — exact `EDGE_IDS` sequence
- A7 | PASS | `verify.txt:136` — no slicing / second glue-up
- A8 | PASS | `verify.txt:136` — buffer string only in `flatten` detail

## End grain (T3)

- A9 | PASS | `verify.txt:136` — exact `END_IDS` sequence
- A10 | PASS | `verify.txt:136` — buffer in `flatten-panel` + `flatten-board` only
- A11 | PASS | `build-steps.ts:223` + `:265` — `metrics.sliceCount`
- A12 | PASS | `verify.txt:136` — `2-step`, `as designed`, `turned 180°`; no raw enum slugs

## Shared correctness

- A13 | PASS | `verify.txt:136` — walnut `repeat: 3` → rip count 3
- A14 | PASS | no `wasteFactor` in `build-steps.ts`
- A15 | PASS | `verify.txt:136` — no `detail` matches `/\d+\.\d+/`
- A16 | PASS | `build-steps.ts:60–67` — `metrics.panelPlan`, skip `rows === 0`
- A17 | PASS | `verify.txt:136` — `tests/board-designer-build-steps.test.ts` (19 tests)

## Regression gate

- R1 | PASS | `verify.txt:223–228` — all four PASS, `=== EXIT: 0 ===`
...
M: 1 PASS (excluded). Round 1 deltas applied.
