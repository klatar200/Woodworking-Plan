...
Re-verify after `ACCEPTANCE.sha256` re-lock — lock now matches ACCEPTANCE.md.
...
- A3 | PASS | `src/lib/board-designer/history.ts:249–250`; `tests/designer-history.test.ts:486–501`; suite green at `verify.txt:53`
- A4 | PASS | `src/lib/board-designer/history.ts:229`; `tests/designer-history.test.ts:504–519`; `verify.txt:53`
...
- A6 | PASS | `verify.txt:30` — `tests/board-designer-metrics.test.ts` (includes dangling-panel warning case)
- A7 | PASS | `tests/designer-history.test.ts:483` — one `add-panel` → `past.length === 1`; `verify.txt:53`
- A8 | PASS | `tests/designer-history.test.ts` describe `add-panel — appends one row step (Sprint 77)`; `verify.txt:53`

## Strip editor placement (T2)

- A9 | PASS | `src/components/designer/strip-list.tsx:198–295` (section) before `:297–412` (`<ol>`)
- A10 | PASS | `tests/strip-drag.test.tsx:242–250`; `verify.txt:162`
...
- A13 | PASS | `tests/strip-drag.test.tsx:253–258`; `verify.txt:162`

## Panel header legibility (T3)

- A14 | PASS | `src/components/designer/panel-editor.tsx:165` — `aria-label={`Delete ${panel.label}`}`; `tests/panel-editor-header.test.tsx:23–47`; `verify.txt:161`
- A15 | PASS | `panel-editor.tsx:164` `className={btnDanger}`; Collapse stays `btnGhost` (`:156`); `verify.txt:161`
...
- A19 | PASS | `panel-editor.tsx:151`; `tests/panel-editor-header.test.tsx:56–57`, `:82` (`1 row` / `N rows`); `verify.txt:161`
...
- A22 | PASS | `tests/board-settings-target.test.tsx:18–76`; `verify.txt:160`
- A23 | PASS | `tests/board-settings-target.test.tsx:81–111`; `verify.txt:160`

## Regression gate

- R1 | PASS | `verify.txt:209–214` — all four PASS, `=== EXIT: 0 ===`
...
A+R: 29 PASS / 0 FAIL = **100%** (≥95%). No R FAIL.
