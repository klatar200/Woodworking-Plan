...
**TOPOLOGY — the three actors do NOT share a working tree.** Claude edits Keagan's local checkout. **Cursor runs in Cursor Cloud** (its own VM, own clone — see `AGENTS.md`): it sees a pack only after Keagan PUSHES, and its work reaches Keagan only after Keagan PULLS. `origin/main` is the transport; the pack is the payload. So every Claude→Cursor handoff needs a push from Keagan first, and every Cursor→Claude handoff needs a pull. Never assume a file Claude just wrote is visible to Cursor, or that Cursor's `SCORECARD` is on disk locally — check, or ask Keagan to pull. Cursor pushes a `cursor/sprint-NN-*` BRANCH, not `main` — so `changes.diff` is cut branch-vs-main and the audit happens BEFORE the merge: `git fetch origin` then `git diff "origin/main...origin/cursor/<branch>" --output=sprints/NN/changes.diff`. Three non-obvious requirements, all learned in Sprint 75: `--output=` not `>` (PowerShell 5.1 redirection writes UTF-16LE and every downstream text tool then fails to parse it), quoted (PS eats a bare `@{` as a hashtable literal), three dots (`A...B` = what B added since diverging). Never `HEAD~1`; never `HEAD@{1}`.
