...
Roadmap (BUILD_PLAN §4 authoritative): Phase 0–3 ✅; Phase 4 🟡 PARTIAL — build logs (27) + tool-aware search (25–26) only; rest CLOSED; UI migration (28–32) CLOSED; UX Remediation (33–42) CLOSED; Notch rebrand (43–45) CLOSED; Sprint 46 (catalog-UX) ✅ shipped 2026-07-23; Sprints 47–50 ✅ shipped; Cutting Board Designer (Sprints 51–78) — 51–52 BOTH CLOSED, 53–56 ALL CLOSED, 57–66 CLOSED, 67–72 TRACK CLOSED + Shell IA post-close audit CLOSED, 73 CLOSED, 74 CLOSED 2026-07-27 at 98/100, 76 CLOSED 2026-07-28 at 97/100, 77 CLOSED at 98/100, 78 CLOSED at 97/100 (last closed). **D5 build-steps track OPEN, 3 sprints:** 78 pure library ✅ → **79 UI surface + print (NEXT)** → 80 polish. `designBuildSteps()` exists and is unrendered. Out of scope until Keagan adds to BUSINESS_PLAN: comments, tool-substitution notes, lumber price sync, plan versioning, offline shopping mode, community submissions.
...
**TOPOLOGY — all three actors share ONE working tree (Keagan's checkout).** Cursor runs LOCALLY in his Cursor, not Cursor Cloud (2026-07-28: Cloud is metered, local is not). So there is no transport and no latency — a pack you write is immediately readable by Cursor, and its `verify.txt`/`SCORECARD.md` are on disk the moment it finishes. No push, no pull, no "not pushed yet". What separates the actors is BRANCHES: Cursor creates `cursor/sprint-NN-<slug>` off `main` and commits there, and is forbidden push/merge/rebase/reset/stash (one tree, no remote copy). Keagan owns the branch boundary — cutting the diff, merging after the audit, pushing. So `changes.diff` is cut branch-vs-main and the audit happens BEFORE the merge: `git diff "main...cursor/<branch>" --output=sprints/NN/changes.diff` (branch name is at the top of `SCORECARD.md`). Three non-obvious requirements, all learned in Sprint 75: `--output=` not `>` (PowerShell 5.1 redirection writes UTF-16LE and every downstream text tool then fails to parse it), quoted (PS eats a bare `@{` as a hashtable literal), three dots (`A...B` = what B added since diverging). Never `HEAD~1`; never `HEAD@{1}`. If `changes.diff` is missing, ask for it rather than reading `src/` — the fallback costs several times the tokens for the same conclusion (Sprint 76). Same UTF-16LE trap now reaches `verify.txt`, which is why `npm run verify -- --out <path>` exists: Node writes UTF-8, no shell involved. Cursor Cloud remains available on request; `AGENTS.md` carries that branch of the procedure.

- **Handoffs are triggers, never prose.** Keagan's whole Cursor prompt is `Run sprint NN` / `Fix sprint NN`; the procedure lives in `AGENTS.md` §"Triggers" which Cursor loads itself. Never hand him an expanded version, never append "and remember to…" — a caveat that matters goes in the `PLAN.md` guardrail for the task it constrains, where it is under the hash lock and survives a context reset. Restating it in chat is how the pasted-prompt loop grows back. Same rule inbound: `Audit sprint NN` = read `changes.diff` + `verify.txt` + `SCORECARD.md` against `ACCEPTANCE.md`, append `FIXES.md` (gaps only, exact remediation), report the grade.
...
- **`lock` validates before it seals** (`checkPack()` in `scripts/verify.mjs`; `check-pack <dir>` runs the same checks without sealing): denominator == count of `A`+`R` ids, gate numerator == `ceil(pct×scored)`, no duplicate/non-contiguous ids, every `file:line` cite resolves to a real file and an in-range line. These are the mechanical defects that cost a round in 76 and 77. **It cannot judge whether a cite points at the RIGHT place** — Sprint 77's `serialize.ts:197` came off the test file and the source is longer than that, so it passes. Cite accuracy is what the cold read in `Check sprint NN` is for; don't let a green `check-pack` talk you out of the pre-flight.
