...
Roadmap (BUILD_PLAN §4 authoritative): Phases 0–3 done; Phase 4 partial (build logs + tool-aware search only; forums/AI-customization/video/metric shut); UI migration (28–32), UX Remediation (33–42), Notch rebrand (43–45) CLOSED; Sprint 46 (catalog-UX) ✅ shipped 2026-07-23; Sprints 47–50 shipped; Cutting Board Designer (Sprints 51–74) CLOSED — 51–52 designer, 53–56 polish, 57–66 core/U6–U7, 67–72 shell/IA track + post-close audit, 73 material-math, 74 wasteFactor schema v3; last closed Sprint 74 (2026-07-27, 98/100). Out of scope until Keagan adds to BUSINESS_PLAN: comments, tool-substitution notes, lumber price sync, plan versioning, offline shopping mode, community submissions.
...
**TOPOLOGY — the three actors do NOT share a working tree.** Claude edits Keagan's local checkout. **Cursor runs in Cursor Cloud** (its own VM, own clone — see `AGENTS.md`): it sees a pack only after Keagan PUSHES, and its work reaches Keagan only after Keagan PULLS. `origin/main` is the transport; the pack is the payload. So every Claude→Cursor handoff needs a push from Keagan first, and every Cursor→Claude handoff needs a pull. Never assume a file Claude just wrote is visible to Cursor, or that Cursor's `SCORECARD` is on disk locally — check, or ask Keagan to pull. `changes.diff` is cut over what the pull brought in (`git diff HEAD@{1}..HEAD`), not `HEAD~1`.
