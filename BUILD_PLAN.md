...
| Sprints 53–56 (Designer polish) | 🟡 Sprint **53 CLOSED** 2026-07-25 — **98/100 at Attempt 2** (`cff58b9` layout/chrome + `c4a4e99` sticky fix); suite **1105/1105**. Attempt 1 shipped `lg:sticky` *and* `lg:items-start`; the latter shrink-wrapped the preview column to 732px, so the preview left the viewport after **130px of a 6749px page** while the suite stayed green (it asserted the class, not the behaviour). Columns stretch now; verified in view at the last strip and at page bottom on prod, and at 2544/1440/1280/1024/900. Remaining −2 is test coverage (source-level guard; jsdom cannot evaluate sticky). 54–56 still scheduled, unopened. U6/U7 still unopened; advanced templates + wider Canva-like set deferred. |

Test suite — expected: 1105/1105 (Sprint 53, Cursor VM). Update this single figure each sprint close; 1104/1104 and earlier superseded.
...
### Sprints 53–56 — Designer polish (SCHEDULED 2026-07-25, Keagan).
Sprints 51–52 delivered U1–U5 and are closed. **U6/U7 remain Phase 2, unopened** — these four sprints are a separate polish track, not U6/U7. Decisions of record: DECISIONS_LOG 2026-07-25 (mobile-first scoping; undo/redo). Order below is the recommendation; priority is Keagan's to reorder.

| Sprint | Scope (Keagan's item #s) | Notes / gates |
|---|---|---|
| **53 — Designer layout & chrome** | **2** full-page width like `/browse` · **3** preview stays in view while editing · **4** `PREVIEW` left + `Export PNG` right on one line · **6** strip `Up`/`Down` verbiage · **7** species pills one line, `Purpleheart` overflow | ✅ **CLOSED** 2026-07-25 — `cff58b9`, score **96/100**. Source+suite verified; Keagan viewport/sticky/species/canvas-size pass still open (see SPRINT_LOG). OrbitControls untouched. |
| **54 — Desktop-only designer + mobile plan/cut list** | **1** | Gate on **viewport width, never UA sniffing**. Mobile keeps `/designer/library`, a read-only `/designer/[id]`, and the print sheet. **B7 is binding:** do NOT satisfy "cut list on mobile" by rendering `toParts()` (Phase 2 / U6). `/designer/[id]/print` already carries the strip table, board feet, dimensions and diagram — make it mobile-legible instead. §6 cat-6 is still scored, against the read-only surface |
...
- **Advanced templates (item 9)** — plaid ≈ representable today (strip widths + `flipEveryOtherSlice`); **zigzag and 3D-box are not**, because `Strip` has no angle and `Cell` is axis-aligned `{xIn,yIn,wIn,hIn}`. Those need a §3 type-contract change + new geometry + new tests (contract §6 rule 3: stop and amend §3, never negotiate it in code).
