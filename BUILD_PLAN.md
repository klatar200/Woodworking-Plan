...
| Sprints 53–56 (Designer polish) | 🔵 SCHEDULED 2026-07-25, none started — 53 layout/chrome · 54 desktop-only + mobile plan/cut list · 55 undo/redo · 56 species expansion. Detail below. U6/U7 still unopened; advanced templates + wider Canva-like set deferred to their own conversations. |
...
### Sprints 53–56 — Designer polish (SCHEDULED 2026-07-25, Keagan). None started.
...
|---|---|---|
| **53 — Designer layout & chrome** | **2** full-page width like `/browse` · **3** preview stays in view while editing · **4** `PREVIEW` left + `Export PNG` right on one line · **6** strip `Up`/`Down` verbiage · **7** species pills one line, `Purpleheart` overflow | No data-model change, no new routes, no new marketing copy. Item 6 is a **real defect**, not just wording: the labels are panel-order but are read against a rotated 3D view — re-word relative to the diagram, never the camera. B6 leaves control labels to engineering judgment. Item 7 confirmed live on prod. **Do this first** — 2+3 make a landscape tablet usable and should be re-judged before 54 narrows the surface |
